...
\item \textbf{Competitive accuracy at a fraction of the representation cost.} From a 128-dimensional representation, DTS+HGB matches the strongest sequence-learning baselines, MultiROCKET (+0.0027) and a resource-matched InceptionTime (+0.0061), neither margin significant, while significantly outperforming GRU (+0.0183), Transformer (+0.0085), MiniROCKET (+0.0135), a TCN (+0.0265), and both ST-GCNs; its point-estimate AUROC (0.9895 [0.9851, 0.9935]) is the highest of the fourteen evaluated target-trained models, and it dominates the originally evaluated baselines at every training-set size. At fixed false-alarm budgets, where alarm fatigue is the binding constraint, DTS+HGB dominates MiniROCKET everywhere (0.796 vs 0.629 recall at 1\% FPR) and trades operating points with MultiROCKET, ahead at the strictest budgets while DTS+HGB leads at the F1 point and 10\%, at statistically indistinguishable ranking.
\item \textbf{Robustness under distribution shift.} Leave-session-out mean AUROC 0.9649; leave-fall-origin mean 0.8961 with the Bed fold hardest, as the QDA model predicts, recoverable by few-shot adaptation (faster than MiniROCKET, on par with MultiROCKET); zero-shot transfer to URFD (0.9200) and uncontrolled-home GMDCSA-24 (0.9633); true leave-one-subject-out on GMDCSA-24 matching MultiROCKET (0.941 vs 0.934, not significant at four subjects).
...
\end{theorem}
Theorem~\ref{thm:impossibility} is elementary, a direct consequence of order-invariance, and its role is to delimit scope: it pins the exact-chance boundary that no order-blind representation can cross. The proof (appendix) is pointwise in $\sigma$, so it covers both uniformly permuted negatives and deterministic permutations such as exact time reversal, and formalises why the order-invariant bag-of-frames classifier sits exactly at chance in Figure~\ref{fig:benchmark} and Table~\ref{tab:bench}, independent of sample size. The complementary positive result is that an order-sensitive statistic escapes this bound, now in finite-sample form:
...
Neural sequence models: a 2-layer LSTM (hidden size 128; 216,193 parameters), a 2-layer GRU (hidden size 128), and a temporal Transformer (4 heads, 2 layers, $d_{\mathrm{model}}=64$), trained on normalised keypoint sequences of shape $(T,34)$. Graph baselines: Compact ST-GCN (244,161 parameters) and a deeper FullST-GCN-COCO (1,107,905 parameters), both using the 17-node COCO adjacency and temporal graph-convolution blocks adapted from ST-GCN \citep{yan2018stgcn}, trained from scratch on FallVision. Convolutional time-series baselines: MiniROCKET \citep{dempster2021minirocket} via aeon \citep{middlehurst2024aeon} on the same hip-centred, torso-normalised 34-channel sequences (fixed maximum length 150), and MultiROCKET \citep{tan2022multirocket} (49,728 features; ridge with validation-selected regularisation). Deep convolutional baselines (JAX implementations \citep{jax2018}): a four-block dilated causal TCN \citep{bai2018tcn} (dilations 1--8, 64 channels, 43,713 parameters) and a \emph{resource-matched} InceptionTime implementation \citep{fawaz2020inceptiontime}: a three-network ensemble of depth-6 Inception networks (bottleneck 32, kernels 40/20/10, 496,129 parameters each; probabilities averaged), trained under the same 20-epoch budget as the other neural baselines rather than the canonical five-network, train-to-convergence setup, and labelled accordingly. Both use Adam, gradient clipping, and validation-AUROC checkpointing. The added baselines consume sequences rebuilt from the raw archives via the released manifest; the rebuild is validated by reproducing DTS+HGB's AUROC (0.9895, exact) and MiniROCKET's (0.9797, within its published interval; the residual difference is attributable to ridge-regularisation selection, validation-selected here versus leave-one-out in the original run, and library version, the MiniRocket transform being deterministic at fixed seed). DTS classifiers: logistic regression, ExtraTrees, random forest, and HistGradientBoosting on the 128-dimensional DTS vector; DTS-Net as above.
...
At the validation-F1 operating point, DTS+HGB also has a favourable threshold trade-off: 18 false negatives and 39 false positives versus MiniROCKET's 34 and 40. The claim remains evaluated-baseline superiority, not dominance over all possible pretrained skeleton-action systems.
...
The data-scarcity claim is tested directly by retraining on stratified subsamples of the strict twin-free training set ($n\in\{250,500,1{,}000,2{,}000,3{,}565\}$; validation, test set, and protocol unchanged): DTS+HGB, the strongest originally evaluated neural and convolutional baselines (Transformer, MiniROCKET), and the strongest added baseline (MultiROCKET, transform refit per subsample). Figure~\ref{fig:sampleeff}: DTS+HGB dominates Transformer and MiniROCKET at every size, with the MiniROCKET margin largest in the scarcest regimes (+0.018 to +0.021 for $n\leq500$), and trades the lead with MultiROCKET (0.9477 vs 0.9501 at $n{=}250$; 0.9828 vs 0.9788 at 2{,}000; 0.9895 vs 0.9868 at full size). A paired bootstrap of the AUROC difference at each size finds no significant difference anywhere along the curve (all five 95\% intervals contain zero; Appendix Table~\ref{tab:lcpaired}); both methods are markedly more sample-efficient than the learned sequence models. This sweep, not the heuristic capacity argument in the appendix, is the main support for the data-efficiency claim.

\begin{figure}[t]
\centering
\includegraphics[width=0.96\columnwidth]{fig1_sample_efficiency.pdf}
\caption{Sample efficiency on the strict twin-free split: test AUROC versus training-set size for DTS+HGB, the strongest originally evaluated baselines (Transformer, MiniROCKET), and the added MultiROCKET (transform refit per subsample; shaded bands are 95\% bootstrap intervals; validation and test sets fixed). DTS+HGB dominates the originally evaluated baselines at every size and trades the lead with MultiROCKET within overlapping intervals.}
\label{fig:sampleeff}
\end{figure}
...
\textbf{GMDCSA-24.} As a second, more realistic external test, we apply the same archived model to GMDCSA-24 \citep{alam2024gmdcsa}, a 4-subject dataset recorded in three natural home settings under varied lighting, clothing, and a low-resolution webcam. On all 160 clips (79 falls, 81 ADL in our parse), zero-shot DTS+HGB reaches AUROC 0.9633 [0.9362, 0.9849]; at the fixed threshold 0.5, recall 0.924 [0.844, 0.965] and precision 0.880 [0.792, 0.933] (10 FP, 6 FN); recall is 0.861 at 5\% FPR and 0.911 at 10\% FPR. Unlike URFD, the default threshold is already usable (full transfer table in Appendix Table~\ref{tab:transfer}). GMDCSA-24 also publishes subject identifiers, enabling the true subject-held-out test below.
...
\section{Limitations}
\textbf{Erratum.} An earlier draft's near-ceiling results were traced to duplicated clips leaking across splits; all numbers use the strict twin-free protocol (artefacts released). \textbf{Subject identity.} FallVision subject IDs are unpublished (leave-session-out is only a proxy); true subject-held-out evidence comes from GMDCSA-24, which has four subjects, so the LOSO comparison is underpowered and stated as a match. \textbf{Baselines.} ST-GCNs are trained from scratch; PoseC3D and pretrained or fine-tuned systems are not evaluated (pretraining reintroduces the large-external-data condition we isolate): an evaluated-baseline comparison, not a state-of-the-art claim. InceptionTime is reported under a resource-matched budget; the canonical five-network train-to-convergence configuration was not run for compute reasons and is left to future work, though a stronger InceptionTime would reinforce rather than threaten the matches-learned-models claim. \textbf{Domain shift.} Bed falls remain hardest (0.7123 zero-shot) and URFD needs threshold calibration; both are quantified, not solved; Bed recovery needs labelled target clips. \textbf{Population.} Subjects are healthy adults aged 22--27; elderly or frail-adult falls may violate the three-phase pattern more often. \textbf{Scope.} PGTS assumes known event mechanics and a low-dimensional discriminative signal, and is not expected to scale to large multi-class recognition; the sub-3\,ms runtime excludes pose estimation; DTS-Net attention is directional evidence, not validated explanation.

\section{Conclusion}
Under the strict twin-free protocol, DTS+HGB matches MultiROCKET and a resource-matched InceptionTime with a far smaller interpretable representation, dominates MiniROCKET at every fixed FPR, and posts the highest point-estimate AUROC among fourteen target-trained models (0.9895). When event mechanics are known and data scarce, physics-guided temporal signatures are a strong, interpretable alternative whose theory delimits its scope.
...
\bottomrule
\end{tabular}
\caption{True leave-one-subject-out AUROC on GMDCSA-24 (4 subjects). DTS+HGB matches the strong modern convolutional time-series baseline MultiROCKET under person-level shift, with a small, non-significant mean-AUROC advantage and zero sequence-learning parameters.}
\label{tab:loso}
\end{table}
...
\input{bench_tasks_table_compact}
\begin{figure}[t]
\centering
